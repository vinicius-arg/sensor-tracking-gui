...
class SerialReader(Thread):
    ...
    def __init__(self, port, baudrate, queue: Queue, force):
        ...
        self.__connect(self._port_name, self._baudrate, self._force__connection)
    ...
    def __can_recv_packet(self):
        return self.packet_queue.qsize() == 0
    

    def __packet_is_valid(self, data):
        return len(data) == PACKET_SIZE


    def __connect(self, port, baudrate=9600, force=False):
    ...
    def __handshake(self, serial: Serial):
    ...
    def run(self):
        ...
        while self.is_running:
            try:
                if self.__sof_found():
                    if self._pause_event.is_set() or not self.__can_recv_packet():
                        self.__wait()
                    else:
                        data = self.serial.read(PACKET_SIZE)
                        packet = Packet.as_data(data)

                        if self.__packet_is_valid(data):
